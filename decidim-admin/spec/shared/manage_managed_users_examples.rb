...
shared_examples "manage managed users examples" do
  ...
  def fill_in_the_impersonation_form
    within "form.new_managed_user_impersonation" do
      fill_in :authorization_handler_document_number, with: "123456789X"
      fill_in :authorization_handler_postal_code, with: "08224"
      page.execute_script("$('#authorization_handler_birthday').siblings('input:first').focus()")
    end
